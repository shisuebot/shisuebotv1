   ...
   case 'pika':
		 ...
		 global.totalchatgruoup
            ...
            client.sendMessage(from, aud, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true});
					...
					break
case 'perfil':
if (!user && !users) return reply (`*Usuario Não Encontrado*.\n\nDigite ${prefix}Daftar nome|idade`)
...
case 'timer':
       ...
       gay = body.slice(13)
		   ...
		   break
                   ...
                   gay = body.slice(13)
		   anu = await fetchJson(`https://arugaz.herokuapp.com/api/howgay`, {method: 'get'})
		   hasil = `*Mas como voçê e burro${rate} olha só sua porcentagem:* ${anu.persen}%`
		   reply(hasil)
		   break
			case 'inteligência':	
                       case 'inteligencia':	
			...
			case 'inter':	
                   ...
                   gay = body.slice(13)
		   anu = await fetchJson(`https://arugaz.herokuapp.com/api/howgay`, {method: 'get'})
		   hasil = `*Mas como voçê é inteligênt${rate} olha só sua porcentagem:* ${anu.persen}%`
		   reply(hasil)
		   break
...
break
		   ...
		   break
				...
				gay = body.slice(13)
		   anu = await fetchJson(`https://arugaz.herokuapp.com/api/howgay`, {method: 'get'})
		   hasil = `*Atenção estamos verificando o nivel de gay no grupo, o nivel é   ${anu.persen}%*`
		   reply(hasil)
		   ...
		   break
		   	    case 'nivelgado':
					...
					gay = body.slice(13)
		   anu = await fetchJson(`https://arugaz.herokuapp.com/api/howgay`, {method: 'get'})
		   hasil = `*Atenção estamos verificando o nivel de gado no grupo, o nivel é   ${anu.persen}%*`
		   reply(hasil)
       break
                 case 'lang':
			...
			tels = body.slice(6)
               ...
               if (data.error) return reply(data.error)
				...
				client.updatePresence(from, Presence.composing) 
					...
					client.sendMessage(from, '╔══✪〘 Mencionando Todos 〙✪══\n╠➥'+teks+'╚═〘 SHISUE BOT Bot 〙', text, {detectLinks: false, quoted: mek})
					break
                        case 'tagall4':
				client.updatePresence(from, Presence.composing) 
					if (!isGroup) return reply(mess.only.group)
	          ...
	          if (!isUser) return reply(mess.only.daftarB)
               ...
               if (anu.error) return reply(anu.error)
                ...
                break
				  ...
				  case 'rg':
					client.updatePresence(from, Presence.composing)
					if (isUser) { reply('você já está registrado')
...
const ups = {numero: sender.split('@')[0], name: "Não registrado", idade: "Não registrado",time1: time,date1: date,contype: "Free",desc: "Não registrado" }
					...
					var idadedu = reg.split("|")[1];
						user.push(sender)
                                              ...
                                              const ups = {numero: sender.split('@')[0], name:nomedu , idade:idadedu,Nome_do_grupo: groupName ,time1: time,contype: "Free",desc: "Não registrado" }
                                                users.push(ups)
						fs.writeFileSync('./database/json/users.json', JSON.stringify(users))
